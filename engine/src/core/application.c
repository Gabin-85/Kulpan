...
b8 application_create(game* game_inst) {
    ...
    KINFO("Kulpan Engine v. %s", KVERSION);
    ...
    if (!ui_text_create(UI_TEXT_TYPE_BITMAP, "Ubuntu Mono 21px", 21, "Hello!", &app_state->test_text)) {
    ...
    if(!ui_text_create(UI_TEXT_TYPE_SYSTEM, "Noto Sans CJK JP", 31, "こんにちは!", &app_state->test_sys_text)) {
        KERROR("Failed to load basic ui system text.");
        return false;
    }
